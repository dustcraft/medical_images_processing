function varargout = mgz_processing(src, niiFolder, fmt)
%% Convert mgz to nii format
% the source code comes from Xiangrui Li's code
% The source code can be downloaded from https://cn.mathworks.com/matlabcentral/fileexchange/42997-dicom-to-nifti-converter--nifti-tool-and-viewer
% I modify the source code as below:
% 1. comment some output information (such as original author information
% (line 491))
% 2. add some my marks
% for example: nii pass & format comment (such as line 923)
% 3. add three output parameters:
% file_details (varargout{2}), nii_data (varargout{3}), original_data (varargout{4})
% 4. only use to process mgz to nii format
% note: other format input and output may be error.
% 5. delete dcmHeaders.mat
% if you want to use dcmHeaders.mat, please se the note part.
% 6. this program will not save nii file, but it can output nii format
% data as parameters.
...
%% Note:
% the modified function has four return parameters: project name (cell);
% file_details (structure); nii_data (structure); original_data (matrix) .

% if you want to save dcmHeaders.mat, please uncomment line 953 (954).

%% Convert dicom and more into nii or img/hdr files. 
% original information part
% 
...
% 
% Several preference can be set from mgz_processing GUI. The preference change will
...
rst3D = (isnumeric(fmt) && fmt>3) || (ischar(fmt) && ~isempty(regexpi(fmt, '3D'))); % nii pass
...
niiFolder = fullfile([getfield(what(niiFolder), 'path') filesep]);% save folder % nii pass
converter = ['mgz_processing.m 20' reviseDate]; % nii pass
...
for k = 1:nFile
    [hh{k}, errStr{k}, dict] = dicm_hdr(fnames{k}, dict); % nii pass
...
if nargout>0, varargout{1} = subj; end % return converted subject IDs % return parameter
...
if ~no_save
    fprintf('Converting %g series (%s) into %g-D %s format data: subject %s\n', ...
...
%         h{i}{1}.RefAcqTimes = (t - min(t)) * 24 * 3600 * 1000; % day to ms
    ...
    nii = nii_tool('init', img); % create nii struct based on img %img is the core data (uint8 format) % nii pass % call nii_tool once
    fname = [niiFolder fnames{i}]; % name without ext %save name % nii pass

    % Compute bval & bvec in image reference for DTI series
    if s.isDTI, [h{i}, nii] = get_dti_para(h{i}, nii); end % nii pass
    
    [nii, h{i}{1}] = set_nii_header(nii, h{i}{1}, pf); % set most nii hdr
    h{i}{1}.ConversionSoftware = converter;
    nii.ext = set_nii_ext(h{i}{1}, pf); % NIfTI extension % nii pass
    if pf.save_json, save_json(h{i}{1}, fname); end % nii pass

    % Save bval and bvec files after bvec perm/sign adjusted in set_nii_header
    if s.isDTI && ~no_save, save_dti_para(h{i}{1}, fname); end

    [nii, niiP] = split_philips_phase(nii, s); % split Philips mag&phase img % nii pass
    ...
    if no_save % only return the first nii
        ...
        fprintf(fmtStr, fnames{i}, nii.hdr.dim(2:5)); % show info and progress % nii pass
        %TEMP = nii; % nii pass
        nii_tool('save', nii, [fname ext], rst3D);  % processe nii & save % nii pass % call nii_tool twice
    end
...
h = cell2struct(h, fnames, 2); % convert into struct % nii pass
fname = [niiFolder 'dcmHeaders.mat']; % nii pass
if exist(fname, 'file') % if file exists, we update fields only % nii pass
    S = load(fname);
...
% return variable s
varargout{2} = h;
% return nii structure
varargout{3} = nii_data;
% return original data
varargout{4} = original_data;
% uncomment two lines below to save dcmHeaders.mat file
%save(fname, 'h', '-v7'); % -v7 better compatibility
%fprintf('Elapsed time by mgz_processing is %.1f seconds\n\n', toc);
return; % nii pass
...
switch cmd
    ...
    case 'about'
        ...
        if item == 1 % about
            str = sprintf(['mgz_processing.m by Xiangrui Li\n\n' ...
                'Feedback to: xiangrui.li@gmail.com\n\n' ...
                'Last updated on 20%s\n'], reviseDate);
            helpdlg(str, 'About mgz_processing')
...
set(fh, 'Toolbar', 'none', 'Menubar', 'none', 'Resize', 'off', 'Color', clr, ...
    ...
    'Name', 'mgz_processing - DICOM to NIfTI Converter', 'NumberTitle', 'off');
...
d = sort({reviseDate('nii_viewer') reviseDate('nii_tool') ...
          reviseDate('mgz_processing') reviseDate('dicm_hdr')});
d = ['20' d{end}];
...
try
    ...
    tmp = [tempdir '/xiangruili-mgz_processing-14ceee6/'];
