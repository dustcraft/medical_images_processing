...
%% nii processing tool
% I modify the save part, if output format is nii and 'save' mode, this function will not
% save it as file, and just return the data to mgz_processing function.
% Other notification, please see note part.
%% Parameter addition part
% add blobal variables to return parameter
global nii_data;
global original_data;

%% note:
%if you want to save nii file, please go to line: 415,437,442-446 and
%455-456 and then uncomment the lines mention above.
...
elseif strcmpi(cmd, 'save')
    if nargin<2, error('nii_tool(''%s'') needs second input', cmd); end
    nii = varargin{1};  %parameter of nii structure % nii pass
    if ~isstruct(nii) || ~isfield(nii, 'hdr') || ~isfield(nii, 'img') % nii pass
        error(['nii_tool(''save'') needs a struct from nii_tool(''init'')' ...
            ' or nii_tool(''load'') as the second input']);
    end
    
    % Check file name to save
    if nargin>2 % nii pass
        fname = varargin{2};
    ...
    if isempty(fext), fext = '.nii'; end % default single .nii file % nii pass
    fname = fullfile(pth, fname); % without file ext % nii pass
    isNii = strcmpi(fext, '.nii'); % will use .img/.hdr if not .nii % nii pass
    
    % Deal with NIfTI version and sizeof_hdr
    niiVer = para.version; % nii pass
    ...
    [nii, fmt] = nii_tool('update', nii); % nii pass
        
    % This 'if' block: lazy implementation to split into 3D SPM files
    if nargin>3 && ~isempty(varargin{3}) && varargin{3} && nii.hdr.dim(5)>1 % nii pass
    ...
    if isNii
        ...
        nii.hdr.magic = sprintf('n+%g%s', niiVer, char([0 13 10 26 10])); % nii pass
        nii.hdr.vox_offset = nii.hdr.sizeof_hdr + 4 + esize; % nii pass
        % uncomment the line below to save nii file
        %fid = fopen([fname fext], 'W'); % nii pass % open save file 
    else
    ...
    % uncomment the lines below to save nii file
    %for i = 1:nExt % nExt may be 0 % nii pass
    ...
    % Write nii image
    % uncomment two lines below to save nii file
    %fwrite(fid, nii.img, fmt);  % write nii % nii pass
    %fclose(fid); % all written % close the written file % nii pass
    %fmt;
    nii_data = nii;
    original_data = nii.img;
...
elseif strcmpi(cmd, 'hdr') % nii pass
    if nargin<2, error('nii_tool(''%s'') needs second input', cmd); end
...
elseif any(strcmpi(cmd, {'img' 'load'})) % nii pass
    if nargin<2, error('nii_tool(''%s'') needs second input', cmd); end
...
elseif strcmpi(cmd, 'ext') % nii pass
    if nargin<2, error('nii_tool(''%s'') needs second input', cmd); end
...
elseif strcmpi(cmd, 'RGBStyle') % nii pass
...
elseif strcmpi(cmd, 'default') % nii pass
...
elseif strcmpi(cmd, 'update') % old img2datatype subfunction % nii pass
    if nargin<2, error('nii_tool(''%s'') needs second input', cmd); end % nii pass
    nii = varargin{1};
    ...
    if isfield(nii, 'ext') % nii pass
        try swap = nii.hdr.swap_endian; catch, swap = false; end % nii pass
